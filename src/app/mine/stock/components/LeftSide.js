...
const LeftSide = () => {
  ...
  const getdata = useSelector(
    (state) => state?.minedata?.stockdata?.stockstore
  );
  const getdata2 = useSelector(
    (state) => state?.minedata?.stockpdata?.stockpstore
  );
  const getdata3 = useSelector(
    (state) => state?.minedata?.stockndata?.stocknstore
  );
  const totalCount = useSelector(
    (state) => state?.minedata?.stockdata?.totalCount
  );
  const totalCount2 = useSelector(
    (state) => state?.minedata?.stockpdata?.totalCount
  );
  const totalCount3 = useSelector(
    (state) => state?.minedata?.stockndata?.totalCount
  );
  ...
  const handlePublish = async (id, newstatus) => {
    try {
      ...
      dispatch(paginationstock({ offset: currentpage * 6, search }));
        window.location.reload();
    } catch (error) {
  ...
  return (
    <div>
      <Banner Title="Stock" Route="Mine" Original="Stock" />
